...
def dataloader(fold, bs=8, b_tfms=Normalize.from_stats(*imagenet_stats)):
    dls = ImageDataLoaders.from_df(df=train_ims,  # specify df holding image names
                                   path='',  # set path for where to find images
                                   item_tfms=RandomResizedCrop(224, min_scale=0.35),
                                   suff='.jpg',  # add the .png suffix to file names from df
                                   label_col='target',
...
b_tfms = [
    *aug_transforms(do_flip=True, flip_vert=True, max_rotate=45.0, max_zoom=1.1, size=224, max_lighting=0.2, max_warp=0.4, p_affine=0.75, p_lighting=0.75, xtra_tfms=None, mode='bilinear'),
...
for fold in range(1, 6):
    ...
    learn = vision_learner(dls,  # specify dataloader object
                           ...
                           model_dir='output')  # specify output location to store model
...
def saving_preds(kaggle_preds=None):
...
def exporting_model(learn=None):
    for fold in range(1, 6):
        learn.load(f'resnet34_rocauc_fold{fold}',
                   device=device,  # ensure the loaded model uses the active cuda:0 device
                   with_opt=True,  # load optimizer state
                   strict=True)
        learn.export(f'melanoma_detector_fold{fold}.pkl')


# print(
#     "Hello! Choose command for starting:\n Data preparing, 1 step\n Data preparing, 2 step\n Data preparing, 3 step\nData preparing, 4 step\nLearning model\nExporting model\nSaving predictions\n")
# commands = {
